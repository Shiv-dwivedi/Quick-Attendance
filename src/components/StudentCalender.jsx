...
const CalendarComponent = ({ onDateChange, studentId, classId }) => {
  ...
  const fetchAttendanceData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/attendance/calendar/${classId}/${studentId}`,
        {
          params: { month, year },
        }
      );

      setAttendanceData(response.data); // Attendance data in the format { 'YYYY-MM-DD': 'present' }
    } catch (error) {
    ...
    return (
      ...
      <>
        ...
        {Array.from({ length: daysInMonth }).map((_, index) => {
          ...
          let dotColor;
          if (attendanceStatus === 'present') dotColor = 'green';
          else if (attendanceStatus === 'absent') dotColor = 'red';
          else if (attendanceStatus === 'leave') dotColor = 'yellow';
          else if (attendanceStatus === 'request') dotColor = 'blue';
